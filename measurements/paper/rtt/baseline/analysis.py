...
def grab_times(pcap: str, src_mac: str, dst_mac: str, filter_str: str) -> List[float]:
...
def main():
    ...
    ap.add_argument("--filter", help="add an adiditional filter to pyshark")
    ap.add_argument("--one-to-one", action="store_true",
                    help="Enforce one-to-one sequential pairing.")
    args = ap.parse_args()

    try:
        t1 = grab_times(args.pcap, args.p1_src, args.p1_dst,args.filter)
        t2 = grab_times(args.pcap, args.p2_src, args.p2_dst,args.filter)
    ...
    deltas_us.sort()
    deltas_us = deltas_us[:len(deltas_us)-1]
    import pprint
    pprint.pprint(deltas_us)
    ...
    print(f"Median  (µs): {fmt_us(median)}")
